...
const int   DCF_PERIOD_MS   =            100; // DCF period in milliseconds
const int   SETNOW_PERIOD_S =             10*60; // set DCF `now` to NTP time period
const int   SYNCED_PERIOD_S =              1; // recheck if synced period
...
const int   NTP_PERIOD_S    =          10*60; // NTP synchronization period in seconds
...
long olddcftime, oldsetnowtime, oldsyncedtime, oldntptime;
int oldsec;
String inputString     =    "";
...
DCF77::tick_t get_current_signal(const DCF77::time_data_t &now) {
  ...
  switch (now.second) {
    ...
    case 15:
      if (now.undefined_uses_backup_antenna_output) {
        return undefined;
      }
      result = now.uses_backup_antenna; break;

    case 16:
      if (now.undefined_timezone_change_scheduled_output) {
        return undefined;
      }
      result = now.timezone_change_scheduled; break;

    case 17:
      if (now.undefined_uses_summertime_output) {
        return undefined;
      }
    ...
    case 21:
      if (now.undefined_minute_output || now.minute.val > 0x59) {
        return undefined;
      }
...
void loop() {
  ...
  telnetCommands();
  delay(1);
  if ((millis() - olddcftime) >= DCF_PERIOD_MS) {
    olddcftime = millis();
    if (flags.synced && flags.setnow) checkSetNow();
    modulate();
  }
  if ((millis() - oldsetnowtime) >= SETNOW_PERIOD_S * 1000) {
    oldsetnowtime = millis();
    flags.setnow = true;
  }
  if ((millis() - oldsyncedtime) >= SYNCED_PERIOD_S * 1000) {
    oldsyncedtime = millis();
    if (!flags.synced) syncNTP();
  }
  if ((millis() - oldntptime) >= NTP_PERIOD_S * 1000) {
    oldntptime = millis();
    flags.synced = false;
  }
}
